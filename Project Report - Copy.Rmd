---
title: "Forecasting Hermès Stock Price Using Monte Carlo Methods and Time Series Analysis"
author: "Sara Mezuri"
date: "2023-04-10"
output: pdf_document
---
...
```{r, fig.width=8, fig.height=6, fig.cap = "Hermès Stock Prices since 1993"}
...
```{r, fig.width=8, fig.height=6, fig.cap = "Hermès Stock Price Time Series"}
...
```{r, fig.width=8, fig.height=6, fig.cap = "Hermès Stock Price Logged Time Series"}
...
```{r, fig.width=8, fig.height=6, fig.cap = "Differenced Time Series"}
...
```{r, fig.width=8, fig.height=6, fig.cap="Autocorrelation and Partial Autocorrelation Functions"}
...
```{r, fig.width=8, fig.height=6, fig.cap="Forecast of ARIMA(0, 2, 1) for 24 months"}
...
```{r, fig.width=8, fig.height=6, fig.cap = "Expected Annual Return over 2 years with risk-free rate 1%"}
...
```{r, fig.width=8, fig.height=6, fig.cap = "A Comparison of Expected Annual Returns over 2 years with Different Risk-Free Rates", echo=FALSE}

horizon <- 2 # investment horizon in years
rf_rate_1 <- 0.005 # risk-free rate 
# The expected return is the average return of the simulated 
# stock price over the investment horizon of 2 years
...
# the compounded annual return
annual_return <- exp(mean(log(1 + return)))^horizon - 1 - rf_rate_1 

# We generate 1000 iterations
sim_iterations <- 1000
sim_results <- data.frame(matrix(NA, nrow = sim_iterations, ncol = 2))
colnames(sim_results) <- c("rf_rate_1")
for (i in 1:sim_iterations) {
  sim_price <- rnorm(1, mean = mean_forecast, sd = sd_forecast)
  sim_return <- (sim_price - mean_forecast)/mean_forecast
  sim_annual_return <- exp(mean(log(1 + sim_return)))^horizon - 1 - rf_rate_1
  sim_results[i, 1] <- sim_annual_return
}
...
# we plot distribution of investment returns
plot_1 <-plot(density(sim_results$rf_rate_1), 
     main = "Distribution with Risk-Free Rate of 0.5%", 
     xlab = "Expected Annual Return")

horizon <- 2 # investment horizon in years
rf_rate_2 <- 0.01 # risk-free rate 
# The expected return is the average return of the simulated 
# stock price over the investment horizon of 2 years
...
# the compounded annual return
annual_return <- exp(mean(log(1 + return)))^horizon - 1 - rf_rate_2 

# We generate 1000 iterations
sim_iterations <- 1000
sim_results <- data.frame(matrix(NA, nrow = sim_iterations, ncol = 2))
colnames(sim_results) <- c("rf_rate_2")
for (i in 1:sim_iterations) {
  sim_price <- rnorm(1, mean = mean_forecast, sd = sd_forecast)
  sim_return <- (sim_price - mean_forecast)/mean_forecast
  sim_annual_return <- exp(mean(log(1 + sim_return)))^horizon - 1 - rf_rate_2
  sim_results[i, 1] <- sim_annual_return
}

# we plot distribution of investment returns
plot_2 <- plot(density(sim_results$rf_rate_2), 
     main = "Distribution with Risk-Free Rate of 1% ", 
     xlab = "Expected Annual Return")

rf_rate_3 <- 0.05 # risk-free rate 
# The expected return is the average return of the simulated 
# stock price over the investment horizon of 2 years
...
# the compounded annual return
annual_return <- exp(mean(log(1 + return)))^horizon - 1 - rf_rate_3

# We generate 1000 iterations
sim_iterations <- 1000
sim_results <- data.frame(matrix(NA, nrow = sim_iterations, ncol = 2))
colnames(sim_results) <- c("rf_rate_3")
for (i in 1:sim_iterations) {
  sim_price <- rnorm(1, mean = mean_forecast, sd = sd_forecast)
  sim_return <- (sim_price - mean_forecast)/mean_forecast
  sim_annual_return <- exp(mean(log(1 + sim_return)))^horizon - 1 - rf_rate_3
  sim_results[i, 1] <- sim_annual_return
}

# we plot distribution of investment returns
plot_3 <-plot(density(sim_results$rf_rate_3), 
     main = "Distribution with Risk-Free Rate of 5%", 
     xlab = "Expected Annual Return")

horizon <- 2 # investment horizon in years
rf_rate_4 <- 0.1 # risk-free rate 
# The expected return is the average return of the simulated 
# stock price over the investment horizon of 2 years
...
# the compounded annual return
annual_return <- exp(mean(log(1 + return)))^horizon - 1 - rf_rate_4 

# We generate 1000 iterations
sim_iterations <- 1000
sim_results <- data.frame(matrix(NA, nrow = sim_iterations, ncol = 2))
colnames(sim_results) <- c("rf_rate_4")
for (i in 1:sim_iterations) {
  sim_price <- rnorm(1, mean = mean_forecast, sd = sd_forecast)
  sim_return <- (sim_price - mean_forecast)/mean_forecast
  sim_annual_return <- exp(mean(log(1 + sim_return)))^horizon - 1 - rf_rate_4
  sim_results[i, 1] <- sim_annual_return
}

# we plot distribution of investment returns
plot_4 <-plot(density(sim_results$rf_rate), 
     main = "Distribution with Risk-Free Rate of 10% ", 
     xlab = "Expected Annual Return")
